...
2.增加一个字段出版日期，类型为date放在出版社字段后面
alter table book add publishtime date after publication;
3.修改鲁迅写的图书出版日期均为2000-10-1
update book set publishtime = '2000-10-01' where author = '鲁迅';
4.其他图书出版日期均为2005-1-1
update book set publishtime = '2005-01-01' where author != '鲁迅';
5.删除价格在60元以上或者40元以下的图书
delete from book where price >60 or price <40;
```
...
select * from cls1 order by age;
select * from cls1 order by age,score desc;
select * from cls1 order by age desc,score desc;
```
